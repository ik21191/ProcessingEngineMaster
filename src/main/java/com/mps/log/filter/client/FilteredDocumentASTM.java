...
public class FilteredDocumentASTM implements Serializable {
	private static final long serialVersionUID = 1L;
	public static final String client = "ASTM";
		...
		try {
			...
			if ((pageType != 0 || imageCount.equalsIgnoreCase("0")) && !ipFlag.trim().equalsIgnoreCase("Exclude")){
				...
				if (userAgent != null) {
					...
					//
					if(!robotFlag.trim().equalsIgnoreCase("NA") && !robotFlag.trim().equalsIgnoreCase("0")) {
						pageView = robotFlag;
					}else{
						startTime = System.currentTimeMillis();
						if (urlString != null) {
						...
						if (!otherUrlFlag.trim().equalsIgnoreCase("NA") && !otherUrlFlag.trim().equalsIgnoreCase("0")) {
				...
				iTracker = 5.0;
				startTime = System.currentTimeMillis();
				//Account STEP1 : get accounts from IP Account Mapper
				...
				//Account STEP 2: getting Institution codes based on User ID, if Institution ID is NULL
				...
				if(pageType != 0 && pageView.equalsIgnoreCase("counter")){
					...
					if(journalPattern != null){
						...
						if(journalPattern.indexOf("{{journal_id}}") != -1 && doc.getString("journal_id")!=null && doc.getString("journal_id").equalsIgnoreCase("-")){
							...
							journalId = MapperPageType.fetchIDFromUrl(temp.trim().toLowerCase(), urlString.trim().toLowerCase(), "{{journal_id}}", 1);
						...
						if(journalPattern.indexOf("{{proceeding_id}}") != -1 ){
							...
							proceedingId = MapperPageType.fetchIDFromUrl(temp.trim().toLowerCase(), urlString.trim().toLowerCase(), "{{proceeding_id}}", 4);
						...
						if(journalPattern.indexOf("{{article_id}}") != -1 && doc.getString("article_id")!=null && doc.getString("article_id").equalsIgnoreCase("-")){
							...
							articleId = MapperPageType.fetchIDFromUrl(temp.trim().toLowerCase(), urlString.trim().toLowerCase(), "{{article_id}}", 1);
						...
						if(journalPattern.indexOf("{{issue_id}}") != -1){
							...
							issueId = MapperPageType.fetchIDFromUrl(temp.trim().toLowerCase(), urlString.trim().toLowerCase(), "{{issue_id}}", 1);
						...
						if(journalPattern.indexOf("{{search_term}}") != -1 && doc.getString("search_term")!=null && doc.getString("search_term").equalsIgnoreCase("-")){
							...
							searchTerm = MapperPageType.fetchIDFromUrl(temp.trim().toLowerCase(), urlString.trim().toLowerCase(), "{{search_term}}", 1);
						...
						if(journalPattern.indexOf("{{session_id}}") != -1 && doc.getString("session_id")!=null && doc.getString("session_id").equalsIgnoreCase("-")){
							...
							sessionId = MapperPageType.fetchIDFromUrl(temp.trim().toLowerCase(), urlString.trim().toLowerCase(), "{{session_id}}", 1);
